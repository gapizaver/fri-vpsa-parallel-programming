...
#define ERR_USER(msg) { fprintf(stderr, "Napaka \"%s\" v datoteki \"%s\", line %d\n", msg, __FILE__, __LINE__); exit(1); }
#define ERR_SYSTEM(msg) { char err[500]; sprintf(err, "Sistemska napaka \"%s\" v datoteki \"%s\", line %d", msg, __FILE__, __LINE__); perror(err); exit(1); }
#define ERR_CL(err_num) { fprintf(stderr, "OpenCL napaka (%d) \"%s\" v datoteki \"%s\", line %d\n", err_num, clGetErrorString(err_num) , __FILE__, __LINE__); exit(1); }
#define ERR_CHECK(retval) { if (retval != CL_SUCCESS) ERR_CL(retval) }
